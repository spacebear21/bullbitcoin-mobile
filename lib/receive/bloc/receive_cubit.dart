...
class ReceiveCubit extends Cubit<ReceiveState> {
  ...
  Future<String> processPayjoinProposal(
    ...
    final pj = await proposal.assumeInteractiveReceiver();
    // Receive Check 2: original PSBT has no receiver-owned inputs
    final pj2 = await pj.checkInputsNotOwned(
    ...
    final inputs = await Future.wait(
      state.walletBloc!.state.wallet!.spendableUtxos().map(inputPairFromUtxo),
    );
  ...
  Future<InputPair> inputPairFromUtxo(UTXO utxo) async {
